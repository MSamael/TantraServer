...
//                       ReadMessage
//
// pRecvBuffer에서 1개 분량의 메시지를 읽어 낸다.
// 읽은 분량에 대해서는, 키워드에 따라 디코딩하고, 체크썸과 대조한다.
// 체크썸이 일치하면 nProcPosition을 갱신한다.
// 처리할 메시지가 있으면 포인터를 리턴한다.
// 처리할 메시지가 없으면  0을 리턴한다. 
// 메시지가 손상되었으면  ErrorCode 1를 리턴한다.
// 심각한 문제가 있을시   ErrorCode 2을 리턴한다. (3이 3차례 이상 반복되면 서버는 접속을 끊는다.)
// 메시지 손상시 가능한한 [무시] 처리를 하고, 서버는 메시지 손상에 게임이
// 멈추지 않도록 작업 한다.
/////////////////////////////////////////////////////////////////////////
...
//                       AddMessage
//
// 메시지를 SendBuffer에 하나 더하고 Encoding하고 pRecvBuffer에서 1개 분량의 메시지를 읽어 낸다.
// 읽은 분량에 대해서는, 키워드에 따라 디코딩하고, 체크썸과 대조한다.
// 체크썸이 일치하면 nProcPosition을 갱신한다.
// 처리할 메시지가 있으면 포인터를 리턴한다.
// 처리할 메시지가 없으면  0을 리턴한다. 
